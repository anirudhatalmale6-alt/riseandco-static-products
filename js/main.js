$(function() {
	...
	$(document).on('submit', '#loyalty-form', function(e){
        ...
        $('.loyalty-form-respond').html("<div class='content-message'><i class='fa fa-refresh fa-spin fa-4x'></i> <h2>Loading..</h2></div>");
         $("#loyalty-form").hide();
        $.ajax({
            url: $('#loyalty-form').attr('action'),
            type: 'post',
            dataType: 'json',
            data: $(this).serialize(),
            success: function(data){
                if (data == true){
                    $('.loyalty-form-respond').html("<div class='content-message'><i class='fa fa-rocket fa-4x'></i> <h2>Successfully Joined</h2> <p>We cant wait to see you next time you are in.  Just tell us your name, phone, or email address and we will bring up your details. <br>No need for a card that you can lose or forget.  We will do the work for you!! </p></div>");
                } else {
                    $('.loyalty-form-respond').html("<div class='content-message'><i class='fa fa-exclamation-circle fa-4x'></i> <h2>Error sending</h2> <p>Try again later.</p></div>");
                }
